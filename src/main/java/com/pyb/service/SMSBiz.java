...
//	protected MailBiz mailBiz;
  ...
  public void ReturnSendSMg(ReturnDataNew returnData, int dtype,
      String telephone, String vclass,Integer type) {
    ...
    try {
      ...
      bsv.setType(type);
      int id = daoFactory.getSms_validateDao().insert(bsv);
      if (id > 0) {
        //插入成功 可以进行短信验证码发送
        //短信验证码发送成功 则把这条数据写入数据库中的by_sms_running
        Sms_running bsr = SMSUtil.sendMsg(verify_code, telephone, vclass,type,id);//调用发送API
        if (bsr != null) {
          //调用短信第三方API成功
          int xx = daoFactory.getSms_runningDao().insert(bsr);
          if (xx > 0) {
  ...
  public void ReturnReSendSMg(ReturnDataNew returnData, int dtype,
      String telephone, String vclass, String verify_list,Integer type) {
    // TODO Auto-generated method stub
    try {
      if ("1".equalsIgnoreCase(vclass)) {
        //用户注册
      ...
      if (System.currentTimeMillis() - time < 2 * 60 * 1000) {
        ...
        if (System.currentTimeMillis() - time > 15 * 60 * 1000) {
          ...
          //验证码的MAD5
          verify_list = DigestUtils.md5Hex(telephone + verify_code);
          ...
          bsv.setV_time(date.getTime());
          int count = daoFactory.getSms_validateDao().updateByKey(bsv);
          if (count != 1) {
            //更新时间失败
            returnData.setReturnData(errorcode_data, "验证码发送失败", null);
