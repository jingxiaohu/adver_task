...
public class Read_channelAction extends BaseV1Controller {
  ...
  public String channel_list(HttpServletRequest request, HttpServletResponse response,
    ...
    try {
      ...
      channelBiz.ReturnChannelList(returnData, param.dtype,param.ui_id,param.page,param.size);
      sendResp(returnData, response);
      return null;

    } catch (Exception e) {
      log.error(
          "Read_channelAction.ReturnChannelList is error  2.21	Read-读取频道信息列表 (APPSDK-JAVA)- P", e);
