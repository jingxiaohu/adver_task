...
public class Read_bannerListAction extends BaseV1Controller {
   ...
   * 获取商品分类列表
   */
  @RequestMapping(value = "/goods/banner_list")
  @ResponseBody
  public String banner_list(HttpServletRequest request, HttpServletResponse response, BaseParam param) {
    ...
    try {
      ...
      goodsBiz.GainBannerList(returnData,param);
      sendResp(returnData, response);
      return null;

    } catch (Exception e) {
      log.error("banner_list is error  获取首页banner图片列表 - P", e);
