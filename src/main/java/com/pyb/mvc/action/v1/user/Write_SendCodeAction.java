...
import com.pyb.mvc.action.v1.user.param.Param_sendcode;
import com.pyb.service.SMSBiz;
...
public class Write_SendCodeAction extends BaseV1Controller {
  ...
  @RequestMapping(value = "/sendcode",name="用户发送验证码")
  @ResponseBody
  public String UserSendCode(HttpServletRequest request, HttpServletResponse response,
    ...
    try {
      ...
      if(param.getType() == null){
    	  ...
    	  if(param.type == 2){
    		  if (!isMobileNO(param.tel)) {
    		        returnData.setReturnData(errorcode_param, " tel is not a right", null);
    		        sendResp(returnData, response);
    		        return null;
    		      }
    	  }
      ...
      String sign_str = getSignature(Constants.getSystemKey(param.dtype), param.dtype, param.tel, param.vclass);
      ...
      smsBiz.ReturnSendSMg(returnData, param.dtype, param.tel, param.vclass,param.type);
      sendResp(returnData, response);
      return null;
  ...
  public String UserReSendCode(HttpServletRequest request, HttpServletResponse response,
    ...
    try {
      ...
      smsBiz.ReturnReSendSMg(returnData, param.dtype, param.tel, param.vclass, param.verify_list,param.type);
      sendResp(returnData, response);
      return null;
  ...
  public String findpass_mail(){
	ReturnDataNew returnData = new ReturnDataNew();
	 try{
		...
		String sign_str = getSignature(Constants.getSystemKey(param.dtype), dtype,email);
