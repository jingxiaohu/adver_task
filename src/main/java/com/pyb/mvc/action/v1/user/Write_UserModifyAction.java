...
import com.pyb.mvc.action.v1.user.param.Param_ModifyPassword;
import com.pyb.mvc.action.v1.user.param.Param_UserModify;
...
public class Write_UserModifyAction extends BaseV1Controller {
  ...
  public String modifyPassword(HttpServletRequest request, HttpServletResponse response,
    ...
    try {
      ...
      if (RequestUtil.checkObjectBlank(param.old_password)) {
        returnData.setReturnData(errorcode_param, " password is null", null);
        sendResp(returnData, response);
        return null;
      }
      if (RequestUtil.checkObjectBlank(param.new_password)) {
        returnData.setReturnData(errorcode_param, " repassword is null", null);
        sendResp(returnData, response);
        return null;
      }
      if (param.old_password.length() < 5 && param.old_password.length() > 20) {
        returnData.setReturnData(errorcode_param, "旧密码长度不正确", null);
        sendResp(returnData, response);
        return null;
      }
      if (param.new_password.length() < 5 && param.new_password.length() > 20) {
          returnData.setReturnData(errorcode_param, "新密码必须大于4位小于20位", null);
          sendResp(returnData, response);
          return null;
       }

      String sign_str = getSignature(Constants.getSystemKey(param.dtype), param.dtype, param.ui_id,param.old_password, param.new_password);
      ...
      userBiz.UpUserModifyPassword(returnData, param.dtype, param.ui_id,param.old_password, param.new_password);
      sendResp(returnData, response);
      return null;

    } catch (Exception e) {
      log.error("UpUserModifyPassword is error  2.21	Read-修改密码(APPSDK-JAVA)- P", e);
  ...
  @ResponseBody
  @Deprecated
  public String change_pass(HttpServletRequest request, HttpServletResponse response,
    ...
    try {
      ...
      userBiz
          .change_userinfo(returnData, param.dtype, param.ui_id, param.avatar, param.avatarFileName,
              param.nickname, param.sex, param.name, 
              param.email);
      sendResp(returnData, response);
      return null;
