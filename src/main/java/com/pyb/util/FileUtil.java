...
/*	public static void deleteAvatorByURl(String imageurlPath) {
	...
	public static String uploadScaleImage(File image, String imageFileName,String file_folder, int maxWidth, int maxHeight,String telephone) throws IOException {
		// 得到文件的上传路径文件夹
		String targetImageFilePath = FileUtil.makeFolder(FileUtil.makeOccasionallyDir(AppProperties.getBaseDir(),file_folder));
		// 得到图片文件的类型
	...
	public static String uploadScaleAvatarImage(File image, String imageFileName,String file_folder, int maxWidth, int maxHeight,String telephone) throws IOException {
		// 得到文件的上传路径文件夹
		String targetImageFilePath = FileUtil.makeOccasionallyDir(AppProperties.getBaseDir(),file_folder);
		// 得到图片文件的类型
		int index = imageFileName.lastIndexOf(".");
		String imageType = imageFileName.substring(index + 1);
		// 得到文件名
	 ...
	 * @throws IOException 
	 */
	public static String uploadScaleAvatarImage(MultipartFile image, String imageFileName,String file_folder, int maxWidth, int maxHeight,String telephone) throws IOException {
		// 得到文件的上传路径文件夹
		String targetImageFilePath = FileUtil.makeOccasionallyDir(AppProperties.getBaseDir(),file_folder);
		// 得到图片文件的类型
		int index = imageFileName.lastIndexOf(".");
		...
		if (maxWidth == 0 && maxHeight == 0) {
			...
			// 缩小并存放图片
			boolean bool = ImageUtil.minifyImage(image.getInputStream(), maxWidth, maxHeight, imageAbsolutePath, imageType);
			if(!bool){
				return null;
			}
			//替换WINDOWS的文件分隔符
	 ...
	 * @throws IOException 
	 */
	public static String uploadScaleImage(MultipartFile image, String imageFileName,String file_folder, int maxWidth, int maxHeight,String pre) throws IOException {
		// 得到文件的上传路径文件夹
		String targetImageFilePath = FileUtil.makeOccasionallyDir(AppProperties.getBaseDir(),file_folder);
		// 得到图片文件的类型
		int index = imageFileName.lastIndexOf(".");
