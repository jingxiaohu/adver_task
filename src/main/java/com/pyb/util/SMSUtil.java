...
//   @Test
	 ...
	 * 发送短信 
	 */
	public static Sms_running sendMsg(String validatecode,String mobile,String vclass,Integer type,long sv_id){
		String msg = "";
		if("1".equalsIgnoreCase(vclass)){
			msg = String.format(formatmsg_reg, validatecode);
		}else if("2".equalsIgnoreCase(vclass)){
		...
		if(type == 1){
			//邮箱
			flag = EmailUtils.sendEmail(mobile, msg);
		    Sms_running bsr = new Sms_running();
			bsr.setSms_content(msg);
			bsr.setSms_date(new Date().getTime()+"");
			bsr.setSms_tel(mobile);
			//1:自定义短信  0:程序员自定义短信  2:邮箱
			bsr.setSms_type(2);
			bsr.setSms_user(mobile);
			bsr.setSv_id(sv_id);
			if(flag){
				//1-成功，0失败
				bsr.setSms_stat(1);
				bsr.setServer_state(1+"");
			}else{
				bsr.setSms_stat(0);
				bsr.setServer_state(0+"");
				log.error("SMSUtil 验证码发送失败:"+mobile);
			}
			return bsr;
