...
//	@Test
  public void testSendPush_fromJSON() {
    ...
    Gson gson = new GsonBuilder()
        .registerTypeAdapter(PlatformNotification.class,
            new InterfaceAdapter<PlatformNotification>())
        .create();
  ...
  private static Logger log = LoggerFactory.getLogger(PushExample.class);
  ...
  public static void testSendIosAlert() {
    ...
    try {
      PushResult result = jpushClient
          .sendIosNotificationWithAlias(alert, new HashMap<String, String>(), "alias1");
      LOG.info("Got result - " + result);
    } catch (APIConnectionException e) {
      LOG.error("Connection error. Should retry later. ", e);
    } catch (APIRequestException e) {
  ...
  public static void testSendWithSMS() {
    ...
    try {
      ...
      PushResult result = jpushClient
          .sendAndroidMessageWithAlias("Test SMS", "test sms", sms, "alias1");
      LOG.info("Got result - " + result);
    } catch (APIConnectionException e) {
      LOG.error("Connection error. Should retry later. ", e);
    } catch (APIRequestException e) {
