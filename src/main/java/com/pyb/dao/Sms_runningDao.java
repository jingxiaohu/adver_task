...
public class Sms_runningDao extends BaseDao{
    ...
    private  String[] carrays ={"Id","sms_tel","sms_content","sms_stat","sms_type","sms_user","sms_user_name","server_state","sms_date","note","sv_id"};
    private  String coulmns ="Id,sms_tel,sms_content,sms_stat,sms_type,sms_user,sms_user_name,server_state,sms_date,note,sv_id";
    private  String coulmns2 ="sms_tel,sms_content,sms_stat,sms_type,sms_user,sms_user_name,server_state,sms_date,note,sv_id";
    ...
    public int insert(Sms_running bean, String TABLENAME2) throws SQLException{
        String sql;
        try{
            sql = "INSERT INTO "+TABLENAME2+" (sms_tel,sms_content,sms_stat,sms_type,sms_user,sms_user_name,server_state,sms_date,note,sv_id) VALUES (:sms_tel,:sms_content,:sms_stat,:sms_type,:sms_user,:sms_user_name,:server_state,:sms_date,:note,:sv_id)";
    ...
    public int insert_primarykey(Sms_running bean, String TABLENAME2) throws SQLException{
        String sql;
        try{
            sql = "INSERT INTO "+TABLENAME2+" (Id,sms_tel,sms_content,sms_stat,sms_type,sms_user,sms_user_name,server_state,sms_date,note,sv_id) VALUES (:Id,:sms_tel,:sms_content,:sms_stat,:sms_type,:sms_user,:sms_user_name,:server_state,:sms_date,:note,:sv_id)";
            SqlParameterSource ps = new BeanPropertySqlParameterSource(bean);
    ...
    public int[] insert(final List<Sms_running> beans, String TABLENAME2) throws SQLException{
        String sql;
        try{
            sql = "INSERT INTO "+TABLENAME2+" (sms_tel,sms_content,sms_stat,sms_type,sms_user,sms_user_name,server_state,sms_date,note,sv_id) VALUES (?,?,?,?,?,?,?,?,?,?)";
    ...
    public List<Sms_running> selectAll(String TABLENAME2) {
        String sql;
        try{
            sql = "SELECT Id,sms_tel,sms_content,sms_stat,sms_type,sms_user,sms_user_name,server_state,sms_date,note,sv_id FROM "+TABLENAME2+" ORDER BY Id";
    ...
    public List<Sms_running> selectLast(int num ,String TABLENAME2) {
        String sql;
        try{
            sql = "SELECT Id,sms_tel,sms_content,sms_stat,sms_type,sms_user,sms_user_name,server_state,sms_date,note,sv_id FROM "+TABLENAME2+" ORDER BY Id DESC LIMIT "+num+"" ;
            return _np.getJdbcOperations().query(sql, new BeanPropertyRowMapper<Sms_running>(Sms_running.class));
        }catch(Exception e){
            //createTable(TABLENAME2);
    ...
    public List<Sms_running> selectGtKey(int Id, String TABLENAME2) {
        String sql;
        try{
            sql="SELECT Id,sms_tel,sms_content,sms_stat,sms_type,sms_user,sms_user_name,server_state,sms_date,note,sv_id FROM "+TABLENAME2+" WHERE Id>:Id";
    ...
    public List<Sms_running> selectByPage(int begin, int num, String TABLENAME2) {
        try{
            String sql;
            sql = "SELECT Id,sms_tel,sms_content,sms_stat,sms_type,sms_user,sms_user_name,server_state,sms_date,note,sv_id FROM "+TABLENAME2+" LIMIT "+begin+", "+num+"";
            return _np.getJdbcOperations().query(sql,new BeanPropertyRowMapper<Sms_running>(Sms_running.class));
        }catch(Exception e){
            //createTable(TABLENAME2);
    ...
    public int updateByKey(Sms_running bean, String TABLENAME2) {
        try{
            String sql;
            sql = "UPDATE "+TABLENAME2+" SET sms_tel=:sms_tel,sms_content=:sms_content,sms_stat=:sms_stat,sms_type=:sms_type,sms_user=:sms_user,sms_user_name=:sms_user_name,server_state=:server_state,sms_date=:sms_date,note=:note,sv_id=:sv_id WHERE Id=:Id";
            SqlParameterSource ps = new BeanPropertySqlParameterSource(bean);
            return _np.update(sql, ps);
        }catch(Exception e){
    ...
    public int[] updateByKey (final List<Sms_running> beans, String TABLENAME2) throws SQLException{
        try{
            String sql;
            sql = "UPDATE "+TABLENAME2+" SET sms_tel=?,sms_content=?,sms_stat=?,sms_type=?,sms_user=?,sms_user_name=?,server_state=?,sms_date=?,note=?,sv_id=? WHERE Id=?";
            return _np.getJdbcOperations().batchUpdate(sql, new BatchPreparedStatementSetter() {
                //@Override
                public int getBatchSize() {
                    return beans.size();
    ...
    public void createTable(String TABLENAME2) throws SQLException{
        try{
            ...
            sql = "CREATE TABLE IF NOT EXISTS `${TABLENAME}` (" +
                 ...
                 "	`sms_tel`  TINYTEXT COMMENT '//varchar(255)    电话或者邮箱'," +
                 "	`sms_content`  TEXT COMMENT '//text    消息内容'," +
                 "	`sms_stat`  INT(11) COMMENT '//int(11)    失败还是成功，0：失败1：成功'," +
                 "	`sms_type`  INT(11) COMMENT '//int(11)    0-程序员自定义短信1-自定义短信2:邮箱'," +
                 "	`sms_user`  VARCHAR(60) COMMENT '//varchar(60)    发送用户'," +
                 "	`sms_user_name`  VARCHAR(60) COMMENT '//varchar(60)    发送用户'," +
                 "	`server_state`  TINYTEXT COMMENT '//varchar(255)    服务端的状态码'," +
                 "	`sms_date`  VARCHAR(40) COMMENT '//varchar(40)    发送时间'," +
                 "	`note`  TINYTEXT COMMENT '//varchar(255)    备注'," +
                 "	`sv_id`  BIGINT(20) COMMENT '//bigint(20)    sms_validate表的主键ID'," +
                 "	PRIMARY KEY (`Id`)" +
