...
public class Sms_validateDao extends BaseDao{
    ...
    private  String[] carrays ={"id","v_tel","v_code","v_list","v_class","v_time","v_time_str","note","type"};
    private  String coulmns ="id,v_tel,v_code,v_list,v_class,v_time,v_time_str,note,type";
    private  String coulmns2 ="v_tel,v_code,v_list,v_class,v_time,v_time_str,note,type";
    ...
    public int insert(Sms_validate bean, String TABLENAME2) throws SQLException{
        String sql;
        try{
            sql = "INSERT INTO "+TABLENAME2+" (v_tel,v_code,v_list,v_class,v_time,v_time_str,note,type) VALUES (:v_tel,:v_code,:v_list,:v_class,:v_time,:v_time_str,:note,:type)";
    ...
    public int insert_primarykey(Sms_validate bean, String TABLENAME2) throws SQLException{
        String sql;
        try{
            sql = "INSERT INTO "+TABLENAME2+" (id,v_tel,v_code,v_list,v_class,v_time,v_time_str,note,type) VALUES (:id,:v_tel,:v_code,:v_list,:v_class,:v_time,:v_time_str,:note,:type)";
            SqlParameterSource ps = new BeanPropertySqlParameterSource(bean);
    ...
    public int[] insert(final List<Sms_validate> beans, String TABLENAME2) throws SQLException{
        String sql;
        try{
            sql = "INSERT INTO "+TABLENAME2+" (v_tel,v_code,v_list,v_class,v_time,v_time_str,note,type) VALUES (?,?,?,?,?,?,?,?)";
    ...
    public List<Sms_validate> selectAll(String TABLENAME2) {
        String sql;
        try{
            sql = "SELECT id,v_tel,v_code,v_list,v_class,v_time,v_time_str,note,type FROM "+TABLENAME2+" ORDER BY id";
    ...
    public List<Sms_validate> selectLast(int num ,String TABLENAME2) {
        String sql;
        try{
            sql = "SELECT id,v_tel,v_code,v_list,v_class,v_time,v_time_str,note,type FROM "+TABLENAME2+" ORDER BY id DESC LIMIT "+num+"" ;
            return _np.getJdbcOperations().query(sql, new BeanPropertyRowMapper<Sms_validate>(Sms_validate.class));
        }catch(Exception e){
            //createTable(TABLENAME2);
    ...
    public List<Sms_validate> selectGtKey(long id, String TABLENAME2) {
        String sql;
        try{
            sql="SELECT id,v_tel,v_code,v_list,v_class,v_time,v_time_str,note,type FROM "+TABLENAME2+" WHERE id>:id";
    ...
    public List<Sms_validate> selectByPage(int begin, int num, String TABLENAME2) {
        try{
            String sql;
            sql = "SELECT id,v_tel,v_code,v_list,v_class,v_time,v_time_str,note,type FROM "+TABLENAME2+" LIMIT "+begin+", "+num+"";
            return _np.getJdbcOperations().query(sql,new BeanPropertyRowMapper<Sms_validate>(Sms_validate.class));
        }catch(Exception e){
            //createTable(TABLENAME2);
    ...
    public int updateByKey(Sms_validate bean, String TABLENAME2) {
        try{
            String sql;
            sql = "UPDATE "+TABLENAME2+" SET v_tel=:v_tel,v_code=:v_code,v_list=:v_list,v_class=:v_class,v_time=:v_time,v_time_str=:v_time_str,note=:note,type=:type WHERE id=:id";
            SqlParameterSource ps = new BeanPropertySqlParameterSource(bean);
            return _np.update(sql, ps);
        }catch(Exception e){
    ...
    public int[] updateByKey (final List<Sms_validate> beans, String TABLENAME2) throws SQLException{
        try{
            String sql;
            sql = "UPDATE "+TABLENAME2+" SET v_tel=?,v_code=?,v_list=?,v_class=?,v_time=?,v_time_str=?,note=?,type=? WHERE id=?";
            return _np.getJdbcOperations().batchUpdate(sql, new BatchPreparedStatementSetter() {
                //@Override
                public int getBatchSize() {
                    return beans.size();
    ...
    public void createTable(String TABLENAME2) throws SQLException{
        try{
            ...
            sql = "CREATE TABLE IF NOT EXISTS `${TABLENAME}` (" +
                 ...
                 "	`type`  INT(11) COMMENT '//int(11)    类型（0:未指定1:邮箱2:手机）'," +
                 "	PRIMARY KEY (`id`)" +
